...
echo "INICIANDO O EXPERIMENTO SOBRE A VERSÃO RELEASE..."
sleep 3
...
for i in `seq 1 101`; do
	echo " "
	echo -e "\e[92m\e[1m*******************************************************"
	echo -e "********** EXECUÇÃO NÚMERO $i (RELEASE MODE) **********"
	echo -e "*******************************************************\e[0m"
	echo " "
	../../build/bin/gmx mdrun -v -deffnm em | grep MO833 >> execution_time_samples.txt
done
...
# Criar arquivo .csv
cut -d\  -f5 < execution_time_samples.txt > execution_time_samples.csv

rm -f execution_time_samples.txt
